...
namespace Aufgabe3_Wolfgang_Ofner
{
    ...
    { 
                        ...
                        else
                        {
                            ...
                            if (input_print.ToUpper().Equals("P"))
                            {
                                do
                                {
                                    ...
                                    Console.WriteLine("\"H\" for help and \"E\" for exit");
                                    Console.ResetColor();

                                    for (int i = 0; i < sorted.Count; i++)
                                    {
                                        sorted[i].Draw(sorted[i]);
                                    }
                                    ...
                                    if (input_move_menu.Contains("."))
                                    {
                                        ...
                                        if (split_name[1].ToUpper().Equals("MOVE"))
                                        {
                                            ...
                                            do
                                            {
                                                ...
                                                Console.Write("Available inputs: ");
                                                Console.ForegroundColor = ConsoleColor.Green;
                                                Console.WriteLine("left, right, up, down, pageup, pagedown, H for Help and E for Exit.");
                                                Console.ResetColor();
                                                ...
                                                switch (input_drawing_move.Key)
                                                {
                                                    ...
                                                    case ConsoleKey.PageDown:
                                                        ...
                                                        if (sorted[index_of_moving_object].Level > 0)
                                                        {
                                                            ...
                                                            Sort(sorted[index_of_moving_object]);           // add the object with the changed level to the list sorted (needed if object1.level == 2, object2.level == 2 than object2.level -1, objects must be switched in the list)
                                                    ...
                                                    case ConsoleKey.PageUp:
                                                        ...
                                                        sorted[index_of_moving_object].Level += 1;      // level + 1 
                                                        Sort(sorted[index_of_moving_object]);           // add the object with the changed level to the list sorted 
                                                    ...
                                                    do
                                                    {
                                                        try
                                                        {
                                                            ...
                                                            if (input_change_new_int >= 0)
                                                            {
                                                                ...
                                                                Sort(sorted[index_of_changing_object]);
        ...
        private static void Create(string type)
        {
            ...
            {
                Console.WriteLine();
                int max_radius_height = ((windowheight - top) / 2) - 1;         // -1 because of space to the bottom border
                int max_radius_width = ((windowwidth - left) / 2) - 1;          // - because of space to the right border
                int max_radius = 0;
                if (max_radius_height >= max_radius_width)
                {
                    max_radius = max_radius_width;
        ...
        private static void Sort(GeometricObject object_xy)
        {
            for (int i = 0; i < sorted.Count; i++)
            {
        ...
        private static void Drawing_Help()
        {
            Console.Clear();
            Console.ForegroundColor = ConsoleColor.DarkYellow;
            Console.WriteLine("This part of the programm draws all your objects. If you want to go back enter \n\"exit\".\n");
            Console.ResetColor();
        }
